...
func (s *server) prepare() OnPrepare {
	return func(cn Connection) context.Context {
		cn.SetOnRequest(s.opt.onRequest)
		cn.SetReadTimeout(s.opt.readTimeout)
		cn.SetIdleTimeout(s.opt.idleTimeout)
		if c, ok := cn.(*connection); ok {
			c.serial = s.opt.serial
		}
		if s.opt.onPrepare != nil {
			return s.opt.onPrepare(cn)
		}
